...
class App:
    ...
    def update(self):
        if self.mode == "title":
            ...
            # BGM 再生（ループ）
            if not pyxel.play_pos(0):  # すでに再生中かチェック
                pyxel.playm(0, loop=True)
            self.update_game()

        elif self.mode == "result":
            # 結果BGMの再生（1回だけ & フレーム遅延で安定）
            if self.play_result_music:
                self.result_music_timer += 1
                if self.result_music_timer == 1:
                    pyxel.stop()
                elif self.result_music_timer == 2:
                    if self.cleared:
